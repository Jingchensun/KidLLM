...
class SupervisedDatasetAudio(Dataset):
    """Dataset for supervised fine-tuning."""

    def __init__(self, data_path: str, audio_root_path: str):
        super(SupervisedDatasetAudio, self).__init__()

        with open(data_path, 'r') as f:
            json_data = json.load(f)

        self.audio_path_list, self.caption_list = [], []
        for item in json_data:
            try:
                one_audio_name, one_caption = item["audio_name"], item["conversation"]
                # TODO: stage 2 dataset format is invalid
                '''
                if not (one_audio_name.lower().endswith('.wav') or one_audio_name.lower().endswith('.mp3')):
                    one_audio_name += '.wav'
                '''
                one_audio_path = audio_root_path + '/{}'.format(one_audio_name)
            except KeyError:
                one_caption = item['conversations']
                one_audio_path = ''
            self.audio_path_list.append(one_audio_path)
            self.caption_list.append(one_caption)
        print(f'[!] collect {len(self.audio_path_list)} samples for training')
    ...
    #def __getitem__(self, i) -> Dict[str, torch.Tensor]: # how to get item, 取一个样本
    def __getitem__(self, i):
        return dict(audio_paths=self.audio_path_list[i], output_texts=self.caption_list[i])

    def collate(self, instances):
        audio_paths, output_texts = tuple([instance[key] for instance in instances] for key in ("audio_paths", "output_texts"))
        return dict(
